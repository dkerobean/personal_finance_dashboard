  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.get(`${backendUrl}/user/messages/`, {
          headers: {
        ...
        >
          ...
          <ul>
            {notifications.map((notification) => (
              <li key={notification.id} className="border-b border-slate-200 last:border-0">
                <Link
                  className="block py-2 px-4 hover:bg-slate-50"
                  to="#0"
                  onClick={() => setDropdownOpen(!dropdownOpen)}
                >
                  <span className="block text-sm mb-2">📣 <span className="font-medium text-slate-800">{notification.content}</span></span>
                  <span className="block text-xs font-medium text-slate-400">{new Date(notification.created_at).toLocaleDateString()}</span>
                </Link>
              </li>
            ))}
          </ul>
